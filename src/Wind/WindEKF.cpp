...
WindEKFGlue::Result
WindEKFGlue::Update(const NMEA_INFO &basic, const DERIVED_INFO &derived)
{
  ...
  float V = basic.TrueAirspeed;
  float dynamic_pressure = V*V;
  float gps_vel[2];
  fixed gps_east, gps_north;
  basic.track.sin_cos(gps_east, gps_north);
  gps_vel[0] = (float)(gps_east*basic.GroundSpeed);
  gps_vel[1] = (float)(gps_north*basic.GroundSpeed);
